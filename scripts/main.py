...
def _read_version_from_pyproject(pyproject_path: str) -> str:
    ...
    try:
        ...
        raise KeyError(
            'Could not find [project].version in pyproject.toml. This action currently supports PEP 621 versions only.'
        ) from e
...
def release(release_type: str) -> int:
    ...
    commit_message = commit_message_template.format(
        tag_prefix=tag_prefix, version=new_version, release_type=release_type
    )
